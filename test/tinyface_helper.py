...
class TinyFaceTest:
    def __init__(self, tinyface_root='/data/data/faces/tinyface', alignment_dir_name='aligned_pad_0.1_pad_high'):
        ...
        self.gallery_dict = scipy.io.loadmat(
            os.path.join(tinyface_root, 'tinyface/Testing_Set/gallery_match_img_ID_pairs.mat'))
        self.probe_dict = scipy.io.loadmat(os.path.join(tinyface_root, 'tinyface/Testing_Set/probe_img_ID_pairs.mat'))
        self.proto_gal_paths = [os.path.join(tinyface_root, alignment_dir_name, 'Gallery_Match', p[0].item()) for p in
                                self.gallery_dict['gallery_set']]
        self.proto_prob_paths = [os.path.join(tinyface_root, alignment_dir_name, 'Probe', p[0].item()) for p in
                                 self.probe_dict['probe_set']]
        self.proto_distractor_paths = get_all_files(
            os.path.join(tinyface_root, alignment_dir_name, 'Gallery_Distractor'))
